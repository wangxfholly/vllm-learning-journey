...
func (s *GatewayServer) Chat(ctx context.Context, req *pb.ChatRequest) (*pb.ChatResponse, error) {
	...
	start := time.Now()
	defer func() {
		telemetry.RequestDuration.WithLabelValues("chat").Observe(time.Since(start).Seconds())
	}()
	...
	_, pickSpan := telemetry.Tracer.Start(ctx, "lb.PickHealthyOrder")
	order := s.lb.PickHealthyOrder()
	pickSpan.SetAttributes(attribute.Int("healthy.count", len(order)))
	pickSpan.End()

	// 实时更新健康实例数 Gauge
	telemetry.HealthyInstances.Set(float64(len(order)))

	if len(order) == 0 {
		log.Printf("[Chat] trace_id=%s 无健康实例,触发降级", traceID)
		span.SetAttributes(attribute.Bool("degraded", true))
		telemetry.DegradedTotal.Inc()
		telemetry.RequestsTotal.WithLabelValues("chat", "degraded", "none").Inc()
	...
	for attempt, inst := range order {
		...
		callCtx, callSpan := telemetry.Tracer.Start(ctx, "vllm.Chat")
		callSpan.SetAttributes(attribute.String("instance.id", inst.ID))
		result, err := inst.Client.Chat(callCtx, messages, req.Temperature, req.MaxTokens)
		if err == nil {
			callSpan.End()
			log.Printf("[Chat] trace_id=%s 实例=%s 成功(第%d次尝试)", traceID, inst.ID, attempt+1)
			telemetry.RequestsTotal.WithLabelValues("chat", "ok", inst.ID).Inc()
			telemetry.TokensTotal.Add(float64(result.CompletionTokens))
	...
	span.SetAttributes(attribute.Bool("degraded", true))
	telemetry.DegradedTotal.Inc()
	telemetry.RequestsTotal.WithLabelValues("chat", "degraded", "all_failed").Inc()
	return s.degradeResponse(traceID), nil
}
